...
namespace PrefixReversal
{
    class PrefixSorting
    {
        ...
        private FixSorting f = new FixSorting();
        ...
        {
            //graph show
            int[] maxSwapArray = new int[m_nMaxSwap];

            for (int i = 0; i < m_nMaxSwap; i++)
            {                
                maxSwapArray[i] = m_SwapArray[i];
            }
            f.run(m_CakeArray, maxSwapArray);
        ...
        private void Search(int step)
        {
            ...
            if (isSorted(m_ReverseCakeArray, m_nCakeCount))
            {
                ...
                for (int k = 0; k < m_nMaxSwap; k++)
                {
                    Console.Write(m_ReverseCakeArraySwap[k]);
                    Console.Write(" ");
                }
                //f.run(m_CakeArray, m_ReverseCakeArraySwap);
                Console.WriteLine();
                return;
            }
